...
void hal_debug_uart_init(uint32_t baudrate)
{
    ...
    debug_uart_handle.Init.StopBits = UART_STOP_BITS_1;
    ...
    gpio_set_alt_function(GPIOB, DEBUG_UART_TX_PIN, GPIO_AF_VALUE_USART2);
    gpio_init(GPIOB, &uart_pin_conf);

    /*configure the GPIO_PORT_B_PIN_11 for the RX functionality */
    uart_pin_conf.pin = DEBUG_UART_RX_PIN;
    gpio_set_alt_function(GPIOB, DEBUG_UART_RX_PIN, GPIO_AF_VALUE_USART2);
    gpio_init(GPIOB, &uart_pin_conf);
}
